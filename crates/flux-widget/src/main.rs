//! Flux widget — the iced daemon: windows, update loop, snapping,
...
// flux.exe (e.g. from the installer or a shortcut) pops a black console
...
const WIDGET_TITLE: &str = "Flux Widget";
...
fn own_window_rects(blocklist: &[String]) -> Vec<(f32, f32, f32, f32)> {
    ...
    unsafe extern "system" fn cb(h: HWND, lp: LPARAM) -> BOOL {
        ...
        // Only dock to Flux's OWN windows (settings/popups), never arbitrary
    ...
    unsafe extern "system" fn cb(h: HWND, lp: LPARAM) -> BOOL {
            ...
            && GetWindowTextLengthW(h) > 0
        {
            ...
            if n > 0 {
                ...
                if !t.starts_with("Flux") && !ctx.titles.contains(&t) { ctx.titles.push(t); }
            }
        }
        BOOL(1)
    }
...
fn set_run_at_startup(on: bool) {
    ...
    if let Ok((key, _)) = hkcu.create_subkey(r"Software\Microsoft\Windows\CurrentVersion\Run") {
        // Clear pre-rename entries first (old "fluxid"/"Fluxid" + C# "fluidMonitor").
        let _ = key.delete_value("fluxid");
        let _ = key.delete_value("Fluxid");
        let _ = key.delete_value("fluidMonitor");
        if on { if let Ok(exe) = std::env::current_exe() { let _ = key.set_value("Flux", &exe.to_string_lossy().to_string()); } }
        else { let _ = key.delete_value("Flux"); }
    }
}
#[cfg(not(target_os = "windows"))]
...
// staying lit.
fn warn_view_for(warnings: &[flux_core::settings::TileWarning], kind: &str, snap: &SensorSnapshot, flash_on: bool) -> WarnView {
...
struct App {
    ...
    // ── Remote monitoring ──
    remote: Option<flux_remote::RemoteManager>,
    remote_rx: Option<std::sync::mpsc::Receiver<flux_remote::RemoteEvent>>,
...
impl App {
    ...
    fn drain_remote_events(&mut self) {
        ...
        for ev in events {
            match ev {
                flux_remote::RemoteEvent::KeyChanged(k) => { self.settings.remote_key = k; }
                flux_remote::RemoteEvent::ConnState { device_id, connected } => {
                    self.remote_conn.insert(device_id, connected);
                }
                flux_remote::RemoteEvent::Snapshot { device_id, snapshot } => {
    ...
    fn device_mut(&mut self, id: &str) -> Option<&mut flux_core::settings::RemoteDevice> {
        self.settings.remote_devices.iter_mut().find(|d| d.id == id)
    }
    fn build_device_from_form(&self) -> Result<flux_core::settings::RemoteDevice, &'static str> {
        ...
        if flux_remote::protocol::decode_handshake_key(key).is_none() { return Err("Invalid handshake key"); }
        Ok(flux_core::settings::RemoteDevice {
            id: new_device_id(), name: name.to_string(), host: ip.to_string(),
            port: self.settings.remote_port, key: key.to_string(),
            popout: flux_core::settings::PopoutSettings::default(),
        })
    }
    fn popout_size(&self, po: &flux_core::settings::PopoutSettings) -> Size {
    ...
    fn save_appearance_to_slot(&mut self, idx: usize) {
        ...
        while self.settings.presets.len() <= idx {
            self.settings.presets.push(flux_core::settings::PresetSlot {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::RemoveInstalledTheme(idx) => {
                if idx < self.settings.installed_themes.len() {
                    self.settings.installed_themes.remove(idx);
                    let _ = self.settings.save();
                }
                Task::none()
            }
            Message::ToggleThemePickerInstalled => {
                self.theme_picker_installed_open = !self.theme_picker_installed_open;
                Task::none()
            }
            ...
            Message::DeletePresetConfirmed => {
                if let Some(slot) = self.confirm_delete_slot.take() {
                    ...
                    if idx < self.settings.presets.len() {
                        self.settings.presets[idx] = flux_core::settings::PresetSlot {
                            name: String::new(), bg: String::new(), tile: String::new(),
                            accent: String::new(), text: String::new(), muted: String::new(), skin: String::new(),
                        };
