...
pub struct AppSettings {
    ...
    #[serde(default = "def_true")] pub gpu_show_clock: bool, // GPU MHz: on now that D3DKMT reports it for AMD/Intel too
...
impl AppSettings {
    ...
    pub fn config_dir() -> PathBuf {
        directories::ProjectDirs::from("com", "Flux", "Flux").map(|d| d.config_dir().to_path_buf()).unwrap_or_else(|| PathBuf::from("."))
    }
    /// Pre-rename config locations, newest first, for one-time migration:
    /// "fluxid"/"Fluxid" (the previous brand) then "fluidMonitor" (the C# app),
    /// so the rebrand to Flux carries over the user's existing settings.
    fn legacy_config_dirs() -> Vec<PathBuf> {
        [("fluxid", "fluxid"), ("Fluxid", "Fluxid"), ("fluidmonitor", "fluidMonitor")]
            .iter()
            .filter_map(|(org, app)| directories::ProjectDirs::from("com", org, app).map(|d| d.config_dir().to_path_buf()))
            .collect()
    }
    /// Copy the most recent pre-rename config into the new Flux location once,
    ...
    pub fn load() -> Result<Self> {
        ...
        match serde_json::from_str(&json) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "flux-core: settings.json was unreadable ({e}); backed up to {} and reset to defaults",
                    bak.display()
                );
                Ok(Self::default())
            }
